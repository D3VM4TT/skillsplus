...
class Attempt extends Component
{
    public function onBeforeSaveAttempt(Entry $entry, Event $event) {
        if ($event->params['isNewEntry']) {
            ...
            if (!is_object($unitEntry) || !Lantra::$app->attempt->canAttempt($entry->authorId, $unitEntry)) {
    ...
    public function canAttempt($userId, $unitEntry) {
        ...
        if (! is_object($unitEntry) || false == $resultEntry = Lantra::$app->result->getUnitResult($userId, $unitEntry->id)) {
            return true;
        }
