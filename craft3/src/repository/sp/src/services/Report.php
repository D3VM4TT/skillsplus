...
class Report extends Component
{
    ...
    public function getCustomReportData($manager, $type, $filter = []) {
        ...
        if ($type == 'users') {
            $values = Lantra::$app->result->getManagerUserSummary($manager->id, $userFilter, $resultFilter);
        }
        elseif ($type == 'results') {
            $resultFilter['resultStatus'] = 'endorsed';
            $displayField = isset($filter['reportDisplayField']) ? $filter['reportDisplayField'] : 'expiryDate';
            $values = Lantra::$app->result->getManagerUserCompletedResults($manager->id, $userFilter, $resultFilter, $displayField);
        }
        elseif ($type == 'expired') {
            ...
            $values = Lantra::$app->result->getManagerUnitExpiredResults($manager->id, $userFilter, $resultFilter, $filter['reportIncludeRequired']);
        }
        elseif ($type == 'required') {
            $values = Lantra::$app->result->getManagerUnitRequiredResults($manager->id, $userFilter, $resultFilter);
        }
        return $values;
    }
    ...
    private function getReportDataResults($userIds, $reportEntry) {
        ...
        $criteria = Lantra::$app->result->getModuleResults($days, null, $expiring, 'complete', null, $userIds);
    ...
    private function getReportDataUsers($reportEntry)
    {
        ...
        foreach($reportEntry->reportCompanies as $company) {
            $companyTeamIds = array_merge($companyTeamIds, Lantra::$app->user->getCompanyTeamIds($company->id));
        }
